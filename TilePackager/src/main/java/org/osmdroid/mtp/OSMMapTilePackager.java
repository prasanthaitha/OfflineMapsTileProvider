...
public class OSMMapTilePackager {
     ...
     public static void main(final String[] args) {
          ...
          if (serverURL == null && isMap == null && !new File(tempFolder).exists()) {
               printUsageAndExit();
          }
          ...
          run(serverURL, isMap, destinationFile, tempFolder, threadCount, fileAppendix, minzoom, maxzoom, north, south, east, west);
     }
      ...
      * @param pWest
      */
     private static void run(final String pServerURL, final String pIsMap, final String pDestinationFile, final String pTempFolder, final int pThreadCount, final String pFileAppendix, final int pMinZoom, final int pMaxZoom, final double pNorth, final double pSouth, final double pEast, final double pWest) {

          new File(pTempFolder).mkdirs();

          System.out.println("---------------------------");
          runFileExpecterWithAbort(pMinZoom, pMaxZoom, pNorth, pSouth, pEast, pWest);
          execute(pServerURL, pIsMap, pDestinationFile, pTempFolder, pThreadCount, pFileAppendix, pMinZoom, pMaxZoom, pNorth, pSouth, pEast, pWest, null);
           if (pServerURL != null) {
     ...
     public static void execute(final String pServerURL, final String pIsMap, final String pDestinationFile,
          ...
          System.out.println("---------------------------");
          if (pServerURL!=null) {
		     runDownloading(pServerURL, pIsMap, pTempFolder, pThreadCount, pFileAppendix, pMinZoom, pMaxZoom, pNorth, pSouth, pEast, pWest, callbackNotification);
	     } else {
...
//               abortIfUserIsNotSure("This will delete the temp folder: " + pTempFolder + " !");
     ...
     private static void runDownloading(final String pServerURL, final String pIsMap, final String pTempFolder, final int pThreadCount,
          ...
          DownloadManager dm;
          if (pIsMap.equalsIgnoreCase("true")) {
               dm = new DownloadManager(pServerURL, pTempBaseURL, pThreadCount);
          } else {
               String baseURL = "http://geoserver.vassarlabs.com:8080/geoserver/gwc/service/wmts?layer=VASSARLABS:"+pServerURL+"&style=VASSARLABS:AP_RIVER_NTW&tilematrixset=EPSG:900913&Service=WMTS&Request=GetTile&Version=1.0.0&Format=image/png&TileMatrix=EPSG:900913:%d&TileCol=%d&TileRow=%d";
               dm = new DownloadManager(baseURL, pTempBaseURL, pThreadCount);
          }
